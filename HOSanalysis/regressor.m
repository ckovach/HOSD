...
classdef regressor 
%
%  Regressor object for point-process and other models. 
% 
%  properties are: 
%
...
%         noptions=[];
    ...
    methods
        ...
        function R = regressor(X,varargin)
        ...
        %                            R1 = regressor(X1,'noptions',noptions);
        %                            R2 = regressor(X2,'noptions',noptions,'codeincr',R2(end).code);
        %                            R3 = regressor(X3,'noptions',noptions,'codeincr',R3(end).code);
        %                            R = [R1,R2,R3];
...
%         R.Npar = npar;
        ...
        %     R.info.COMMAND =  fread(fid);
            R.info.version =  sprintf(['----------- SVN REVISION INFO ------------------',...
                            '\n$URL: https://saccade.neurosurgery.uiowa.edu/svn/GazeReader/0.1/regressor.m $',...
                            '\n$Revision: 98 $',...
                            '\n$Date: 2011-09-13 16:52:46 -0500 (Tue, 13 Sep 2011) $',...
                            '\n$Author: ckovach $',...
                            '\n------------------------------------------------',...
...
%         R.info.functionInputCodes = cat(1,R.code*ones(1,nargin(@R.makefun)),ones(1,nargin(R.makefun)));
        ...
        function Xout = makefun(me,X)
         ...
         function lbls = columnLabels(me)
            if isempty(me.colLabels)
                lbls = strcat(me.label,{':'},arrayfun(@num2str,1:me.Npar,'uniformoutput',false));
            else
                lbls = me.colLabels;
            end
         end
