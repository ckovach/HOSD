...
if isa(S,'dbt')
    ...
    fs = {dbx.frequency};
    mint = round(min(arrayfun(@(d)d.time(end),dbx))*dbx(1).sampling_rate);
    S={};
    for k = 1:order
        S{k} = dbx(k).blrep(round(dbx(k).time*dbx(1).sampling_rate)<=mint,:);
    end
    varargin(1)=[];
else
    f = varargin{1};
...
while i <= length(varargin)
    if isstruct(varargin{i})
        ...
        for k = 1:length(fldn)
%             if ~ismember(fldn{k},cat(1,optfld,ignore_opts))
...
if length(options.lowpass)<order
    options.lowpass(end+1:order) = options.lowpass(end);
end

if length(options.highpass)<order
    options.highpass(end+1:order) = options.highpass(end);
end
...
if options.stats
   %Skipping every other window so that only non-overlapping windows are included in the analysis.
   [stats.beta,stats.dev,stats.pval,stats.iXX,stats.sigma,res,Yfit,df] = complexglm(PS(2:2:end,:),[]);
   stats.beta(end+1)=nan;
   stats.beta = stats.beta(rmat);
   stats.dev(end+1)=nan;
   stats.dev = stats.dev(rmat);
   stats.pval(end+1)=nan;
   stats.pval = stats.pval(rmat);
   stats.sigma(end+1)=nan;
   stats.sigma(end+1) = nan;
   stats.sigma = stats.sigma(rmat);
   out.stats = stats;
end
if nargout >1
    if axes_interchangeable
